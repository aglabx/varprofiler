...
```bash
python plot_chromosomes.py <input.bed> [-o output_dir] [-g gff_file] [-k kmer_size]
```
...
- `-k, --kmer-size`: K-mer size used in analysis (default: 23)

**Examples:**
```bash
# Basic visualization with default k=23
python plot_chromosomes.py kmer_counts.bed -o chromosome_plots

# With satellite DNA annotations overlay and custom k-mer size
python plot_chromosomes.py kmer_counts.bed -o chromosome_plots -g satellites.gff -k 31
```
